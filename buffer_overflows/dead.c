...
/*
Compile: 
`gcc -z execstack -m32 dead.c -fno-stack-protector -o dead -no-pie` 
...
int main(){
	...
	if(my_data.special_int == 0xdeadbeef){
		print_flag(); 
	}
	else{
		puts("Lit :fire");
...
uid_t get_priv_uuid(){
	...
	// User we're trying to elevate too.
   	s = getpwnam_r("deadPriv", &pwd, buf, bufsize, &result);
    if (result == NULL) {
        if (s == 0)
            printf("Not found\n");
        else {
...
void setup_permissions(){ // Ignore this...
	...
	if(current_user == 1002 && geteuid() == 1002){
		return; 
	}
