...
void nested(){
    ...
    /*
    Old Frame pointer (EBP) from 'main'
    Old Stack pointer (ESP) from 'main'
    Old Instruction pointer (EIP) from 'main'
    ------------------------
    */

   	// Clear 'edx' - ignore this...
	__asm__("xor %edx, %edx\n\t");
...
uid_t get_priv_uuid(){
	...
	// User we're trying to elevate too.
   	s = getpwnam_r("execStackPriv", &pwd, buf, bufsize, &result);
    if (result == NULL) {
        if (s == 0)
            printf("Not found\n");
        else {
...
void setup_permissions(){ // Ignore this...
	...
	if(current_user == 1008 && geteuid() == 1008){
		return; 
	}
