...
def add_frame_data(call_id, curRecordings, lastUpdate, consecutive=0):
    # Holds the state of all the phone calls
    con = sqlite3.connect("calls.db") 
    cur = con.cursor()
    cur.execute(
        """
        UPDATE inProgressCall SET curRecording = ?, lastUpdate = ?, consecutive = ? WHERE id = ?
        """, [curRecordings,lastUpdate, consecutive, call_id]
    )
